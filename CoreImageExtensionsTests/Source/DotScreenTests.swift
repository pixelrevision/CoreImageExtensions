...
class DotScreenTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = DotScreen(width: 6, image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, sharpness: 0.7, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CIDotScreen")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.dotScreenFilter(6, sharpness: 0.7, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)

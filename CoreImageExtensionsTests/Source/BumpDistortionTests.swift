...
class BumpDistortionTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = BumpDistortion(image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, center: CIVector(values: [150.0, 150.0], count: 2), scale: 0.5, radius: 300)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CIBumpDistortion")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.bumpDistortionFilter(CIVector(values: [150.0, 150.0], count: 2), scale: 0.5, radius: 300)

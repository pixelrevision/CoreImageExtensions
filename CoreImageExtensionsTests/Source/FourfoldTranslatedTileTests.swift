...
class FourfoldTranslatedTileTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = FourfoldTranslatedTile(width: 100, image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, acuteAngle: 1.570796326794897, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CIFourfoldTranslatedTile")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.fourfoldTranslatedTileFilter(100, acuteAngle: 1.570796326794897, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)

...
class EightfoldReflectedTileTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = EightfoldReflectedTile(width: 100, image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CIEightfoldReflectedTile")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.eightfoldReflectedTileFilter(100, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)

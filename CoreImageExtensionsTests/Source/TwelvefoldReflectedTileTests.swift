...
class TwelvefoldReflectedTileTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = TwelvefoldReflectedTile(width: 100, image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CITwelvefoldReflectedTile")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.twelvefoldReflectedTileFilter(100, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)

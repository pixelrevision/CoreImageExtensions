...
class CrystallizeTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = Crystallize(image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, center: CIVector(values: [150.0, 150.0], count: 2), radius: 20)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CICrystallize")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.crystallizeFilter(CIVector(values: [150.0, 150.0], count: 2), radius: 20)

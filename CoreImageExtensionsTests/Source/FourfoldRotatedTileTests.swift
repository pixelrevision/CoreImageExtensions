...
class FourfoldRotatedTileTests: XCTestCase {

	func testDataStructure() {
		let dataStructure = FourfoldRotatedTile(width: 100, image: CIImage(name: "sample_image_1", classForBundle: self.dynamicType)!, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)
		XCTAssertNotNil(dataStructure.filter())

		let compareFilter = CIFilter(name: "CIFourfoldRotatedTile")
	...
	func testImagefilter() {
		let newImage = CIImage(name: "sample_image_1", classForBundle: self.dynamicType)?.fourfoldRotatedTileFilter(100, center: CIVector(values: [150.0, 150.0], count: 2), angle: 0)
